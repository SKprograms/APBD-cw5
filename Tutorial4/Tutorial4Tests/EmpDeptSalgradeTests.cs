...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnAllSalesmen()
    {
        ...
        List<Emp> result = emps.Where(emps=>emps.Job.Equals("SALESMAN")).ToList();
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = (from e in emps from s in grades where e.Sal >= s.Losal && e.Sal <= s.Hisal select new { e.EName, s.Grade }).ToList();
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(emp => emp.DeptNo).Select(g=> new {DeptNo=g.Key, AvgSal = g.Average(emp => emp.Sal)});  
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.Where(e => e.Sal > emps.Where(em=>em.DeptNo == e.DeptNo).Average(e=>e.Sal)).Select(e=> e.EName);
